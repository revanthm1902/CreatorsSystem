...
-- 17. RPC: Helper to find a user by email in auth.users (for orphan recovery)
-- Returns the user_id if found, and whether they already have a profile
...
BEGIN
  ...
  SELECT id INTO found_user_id FROM auth.users WHERE email = p_email;
  IF found_user_id IS NULL THEN
    RETURN json_build_object('exists', false);
  END IF;

  SELECT EXISTS(SELECT 1 FROM public.profiles WHERE id = found_user_id) INTO has_profile;
...
-- 18. RPC: Admin/Director can set up a new user (confirm email + create profile)
-- Called AFTER the user has been created via GoTrue signUp() to:
--   a) Immediately confirm their email (bypass confirmation flow)
--   b) Create their profile record with the correct role
CREATE OR REPLACE FUNCTION public.admin_setup_user(
  p_user_id UUID,
  p_full_name TEXT,
  p_role TEXT
)
...
BEGIN
  -- 1. Authorization: only Directors and Admins can set up users
  SELECT role INTO caller_role FROM public.profiles WHERE id = auth.uid();
  IF caller_role IS NULL OR caller_role NOT IN ('Director', 'Admin') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE id = p_user_id) THEN
    RAISE EXCEPTION 'Auth user not found. SignUp may have failed.';
  END IF;

  -- 2. Confirm the user's email immediately (bypass email confirmation flow)
  UPDATE auth.users
  SET email_confirmed_at = COALESCE(email_confirmed_at, now()),
      updated_at = now(),
      raw_user_meta_data = COALESCE(raw_user_meta_data, '{}'::jsonb) || jsonb_build_object('full_name', p_full_name, 'role', p_role)
  WHERE id = p_user_id;

  -- 3. Create profile (employee_id auto-generated by trigger)
  ...
  VALUES (p_user_id, p_full_name, p_role, 0, true)
  ON CONFLICT (id) DO UPDATE SET
    full_name = EXCLUDED.full_name,
    role = EXCLUDED.role,
    is_temporary_password = true;

  -- 4. Get the auto-generated employee_id
  SELECT employee_id INTO new_employee_id FROM public.profiles WHERE id = p_user_id;
...
-- 19. RPC: Admin/Director can delete a user (handles FK constraints + auth.users)
...
-- 20. Fix PostgREST schema introspection:
